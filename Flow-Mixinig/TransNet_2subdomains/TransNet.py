...
def plot(xy, u0, x_meshgrid, y_meshgrid, lb, ub, title):
    ...
    plt.tight_layout()
    # plt.savefig('mix_' + str(title) + '.eps')
    plt.show()
...
def load_net_para(net):
    net.load_state_dict(torch.load('model_parameter.pkl'))
...
if __name__ == '__main__':
    ...
    initial_u0 = None
    txy_pred = None
    txy_pred, U_pred = main(lb, ub, N, dt, layers, t_test_index, txy_pred,
                                                           initial_u0, 1)
    ...
    ub = np.array([4, 4, 4])
    t_test_index = [4]
    txy_pred, U_pred = main(lb, ub, N, dt, layers, t_test_index, txy_pred, U_pred,
                                                           2)