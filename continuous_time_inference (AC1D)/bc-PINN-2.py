...
class PhysicsInformedNN:
    def __init__(self, x0, u0, tb, X_f, X_star, u_star, layers, lb, ub):
        ...
        self.t_star_tf = tf.placeholder(tf.float32, shape=[None, self.t_star.shape[1]])
        self.u_star_tf = tf.placeholder(tf.float32, shape=[None, self.u_star.shape[1]])
        ...
        self.loss_ub = tf.reduce_mean(
            tf.square(self.u_lb_pred - self.u_ub_pred) + tf.abs(self.u_lb_pred - self.u_ub_pred)) + \
                       tf.reduce_mean(
                           tf.square(self.u_x_lb_pred - self.u_x_ub_pred) + tf.abs(self.u_lb_pred - self.u_ub_pred))
        ...
        for it in range(nIter):
            if it <= nIter / 10:
                ...
                if it % 10 == 0:
                    ...
                    print(
                        'It: %d, Loss: %.3e, Loss_ui: %.3e, Loss_ub: %.3e, Loss_ur: %.3e, Time: %.2f, step_star: %d, step: %d' %
...
class bcPhysicsInformedNN:
    def __init__(self, x0, u0, t0, u1, tb, X_old, X_f, X_star, u_star, layers, lb, ub, weights, biases):
        ...
        self.x_star_tf = tf.placeholder(tf.float32,
                                        shape=[None, self.x_star.shape[1]])  # MODIFIED FOR I.C at all time steps
        self.t_star_tf = tf.placeholder(tf.float32,
                                        shape=[None, self.t_star.shape[1]])  # MODIFIED FOR I.C at all time steps
        self.u_star_tf = tf.placeholder(tf.float32,
                                        shape=[None, self.u_star.shape[1]])  # MODIFIED FOR I.C at all time steps
...
if __name__ == "__main__":
    ...
    N0 = 200
    x0 = np.linspace(-1, 1, N0).flatten()[:, None]
    u0 = (x0 ** 2) * np.cos(np.pi * x0)
    u0 = u0.reshape(x0.shape)
    ...
    for i in range(0, steps_lb.size - 2):
        t1 = steps_lb[i]
        t2 = steps_ub[i + 1]
        temp_t = t[:t2, :]
        t = t[t1:t2, :]
        ...
        if counter == 0:
            ...
            globals()[f"model_{counter}"] = bcPhysicsInformedNN(x0, u0, t0, u1, tb, X_old, X_f, X_star, u_star, layers,
                                                                lb, ub, W, b)
            start_time = time.time()
            globals()[f"model_{counter}"].train(iterations)
            elapsed = time.time() - start_time
            ...
            if counter == 1:
                pred_test = globals()[f"model_{counter}"].predict(xt_test)
                Exact_u = Exact_u.flatten()[:, None]
                error_u = np.linalg.norm(Exact_u - pred_test, 2) / np.linalg.norm(Exact_u, 2)
