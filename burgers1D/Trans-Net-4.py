...
def plot(x, Exact, U_pred, time):
    ...
    ax.set_xlabel('$x$')
    ax.set_ylabel('$u(t,x)$')
    ax.set_title('t=0.'+str(time), fontsize=10)
    ax.set_xlim([-1.1, 1.1])
    ax.set_ylim([-1.1, 1.1])
    ax.legend(loc='upper right', fontsize=10)
    plt.tight_layout()
    # plt.savefig('./loss_data/figure/Burgers1Dt=0.'+ str(time) + '.eps')
    plt.show()
...
def main(subdomain, N_0, N_b, N_f, layers, t_idx, xt_boundary=None, u_boundary=None):
    ...
    X_star_plot = np.hstack((X.flatten()[:, None], T.flatten()[:, None]))
    ...
    # predict_figure
    U_pred, _ = model.predict(X_star_plot)
    U_pred = griddata(X_star_plot, U_pred.flatten(), (X, T), method='cubic')
    if subdomain == 1:
        plot(x, Exact, U_pred, 15)
    if subdomain == 4:
        plot(x, Exact, U_pred, 75)
...
if __name__ == "__main__":
    ...
    t_idx = [50, 75]
    xt_boundary, u_boundary, u3_pred, u3_star = main(3, N_0, N_b, N_f, layers, t_idx, xt_boundary, u_boundary)

    t_idx = [75, 99]
    xt_boundary, u_boundary, u4_pred, u4_star = main(4, N_0, N_b, N_f, layers, t_idx, xt_boundary, u_boundary)

    u_pred = np.vstack((u1_pred, u2_pred, u3_pred, u4_pred))
    u_star = np.vstack((u1_star, u2_star, u3_star, u4_star))
    error_u = np.linalg.norm(u_star - u_pred, 2) / np.linalg.norm(u_star, 2)
